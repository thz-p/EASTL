...
// 这段代码实现了一个名为 `unique_ptr` 的模板类，用于管理动态分配的对象指针，确保只有一个 `unique_ptr` 实例可以拥有对应的指针。以下是代码的主要特点和功能：

// 1. `unique_ptr` 类的定义和实现，包括构造函数、析构函数、移动构造函数、移动赋值运算符等。
// 2. `unique_ptr` 支持自定义删除器，可以传递自定义的删除器来释放指针。
// 3. 提供了 `reset` 函数用于释放当前指针并接管新的指针，以及 `release` 函数用于释放当前指针但不接管它。
// 4. 支持获取指针、获取删除器等操作。
// 5. 提供了对于 `unique_ptr` 对象之间的比较操作符的重载，以及与 `nullptr` 的比较操作符的重载。
// 6. 包含了 `make_unique` 函数用于创建 `unique_ptr` 对象，可以创建单个对象或动态数组对象。
// 7. 实现了 `swap` 函数用于交换两个 `unique_ptr` 对象的指针。
// 8. 提供了对于数组对象的特化版本 `unique_ptr<T[], Deleter>`，支持通过 `operator[]` 访问数组元素。
// 9. 最后，代码包含了一些与代码风格和规范相关的注释，以及对于不同编译器特性的处理。